...
export function registerMessagingTools(server) {
    server.registerTool("send_message", {
        ...
        description: "Send a text message via a Telegram bot. Supports Markdown and HTML formatting. " +
            "If chat_id is omitted, sends to the default chat (Paulo).",
        inputSchema: {
            text: z
                .string()
                .min(1)
                .describe("Message text content. Supports Markdown or HTML based on parse_mode."),
            bot: z
                .string()
                .optional()
                .describe("Bot name to send from (e.g. 'vendas', 'ads'). Omit for default bot."),
            chat_id: z
                .string()
                .optional()
                .describe("Telegram chat ID to send to. Omit to use the default TELEGRAM_CHAT_ID."),
            parse_mode: z
                .enum(["MarkdownV2", "HTML"])
                .optional()
                .describe("Message formatting: 'MarkdownV2' or 'HTML'. Omit for plain text."),
        ...
        try {
            ...
            return toolError(`Failed to send message: ${error.message}`);
        }
    });
    server.registerTool("send_photo", {
        title: "Send Telegram Photo",
        description: "Send a photo via a Telegram bot. Photo must be a publicly accessible URL or a Telegram file_id.",
        inputSchema: {
            photo: z
                .string()
        ...
        try {
            ...
            return toolError(`Failed to send photo: ${error.message}`);
        }
    });
    server.registerTool("send_document", {
        title: "Send Telegram Document",
        description: "Send a document/file via a Telegram bot. Document must be a publicly accessible URL or a Telegram file_id.",
        inputSchema: {
            document: z
                .string()
                .describe("Document URL (publicly accessible) or Telegram file_id."),
            bot: z
                .string()
                .optional()
                .describe("Bot name to send from. Omit for default bot."),
        ...
        try {
            ...
            return toolError(`Failed to send document: ${error.message}`);
        }
    });
}
