...
export function registerUpdateTools(server) {
    server.registerTool("get_updates", {
        ...
        description: "Get recent incoming messages/updates for a bot. Uses short polling. " +
        ...
        inputSchema: {
            ...
            offset: z
                .number()
                .optional()
                .describe("Update offset. Pass last update_id + 1 to get only new updates."),
        },
        annotations: {
            readOnlyHint: true,
        ...
        try {
            ...
            const data = await apiRequest("getUpdates", bot, undefined, queryParams);
            return toolResult(data);
        }
        catch (error) {
            return toolError(`Failed to get updates: ${error.message}`);
        }
    });
    server.registerTool("list_bots", {
        title: "List Configured Bots",
        description: "List all configured Telegram bots and their names. " +
        ...
        try {
            ...
            const results = await Promise.all(bots.map(async (b) => {
                try {
                    const me = await apiRequest("getMe", b.name);
                    return {
                        name: b.name,
                        telegram_id: me.id,
            ...
            return toolError(`Failed to list bots: ${error.message}`);
        }
    });
}
